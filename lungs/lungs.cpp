...
//  *   2D/3D Transforms    (Ch.3) — mirror + scale for left/right lung
...
// /* ─────────────────────────────────────────────────────────────
//    STAGE COLOUR TABLES  (index 0=healthy … 5=critical)
// ───────────────────────────────────────────────────────────── */
// static const Col3 LC[6]={   /* lung fill */
//     {0.82f,0.18f,0.20f},{0.72f,0.17f,0.18f},{0.52f,0.20f,0.12f},
//     {0.36f,0.15f,0.09f},{0.20f,0.09f,0.06f},{0.08f,0.04f,0.03f}};
// static const Col3 LH[6]={   /* highlight */
//     {0.98f,0.55f,0.55f},{0.88f,0.42f,0.38f},{0.65f,0.30f,0.20f},
//     {0.40f,0.16f,0.11f},{0.22f,0.10f,0.07f},{0.10f,0.05f,0.04f}};
// static const Col3 LS[6]={   /* shadow */
//     {0.52f,0.08f,0.09f},{0.44f,0.07f,0.07f},{0.30f,0.08f,0.05f},
//     {0.18f,0.05f,0.04f},{0.10f,0.03f,0.02f},{0.04f,0.02f,0.01f}};
// static const Col3 BC[6]={   /* bronchi */
//     {0.92f,0.90f,0.87f},{0.80f,0.77f,0.73f},{0.60f,0.56f,0.50f},
//     {0.40f,0.34f,0.26f},{0.24f,0.18f,0.13f},{0.14f,0.10f,0.07f}};
// static const Col3 SC[6]={   /* stage HUD */
//     {0.15f,0.65f,0.22f},{0.75f,0.65f,0.05f},{0.82f,0.45f,0.05f},
//     {0.80f,0.22f,0.06f},{0.72f,0.10f,0.07f},{0.38f,0.04f,0.04f}};

// /* ── Colour helpers ─────────────────────────────────────────── */
// Col3 LungSimulation::lungFill()   const {
//     int s=anim.stage; if(s>=5)return LC[5];
//     return R_lerp(LC[s],LC[s+1],anim.stageFrac()); }
// Col3 LungSimulation::lungHigh()   const {
//     int s=anim.stage; if(s>=5)return LH[5];
//     return R_lerp(LH[s],LH[s+1],anim.stageFrac()); }
// Col3 LungSimulation::lungShadow() const {
//     int s=anim.stage; if(s>=5)return LS[5];
//     return R_lerp(LS[s],LS[s+1],anim.stageFrac()); }
// Col3 LungSimulation::bronchi()    const {
//     int s=anim.stage; if(s>=5)return BC[5];
//     return R_lerp(BC[s],BC[s+1],anim.stageFrac()); }
// Col3 LungSimulation::stageCol()   const {
//     return SC[anim.stage<6?anim.stage:5]; }

// /* ── Lifecycle ──────────────────────────────────────────────── */
// void LungSimulation::reset(){
//     anim.reset(); smoke.reset();
// }
...
//     smoke.update(dt);
// }
// void LungSimulation::onKeyRight(){
//     if(anim.stage<LUNG_STAGE_COUNT-1){anim.stage++;anim.stageTimer=0;}
// }
// void LungSimulation::onKeyLeft(){
//     if(anim.stage>0){anim.stage--;anim.stageTimer=0;}
// }
...
//     Col3 sc=stageCol();
//     R_col(sc.r*0.25f,sc.g*0.25f,sc.b*0.25f);
//     R_filledRect(-2.f,0.86f,2.f,1.f);
//     R_col(sc.r*0.55f,sc.g*0.55f,sc.b*0.55f);
//     R_filledRect(-2.f,0.84f,2.f,0.87f);

//     R_col(0.11f,0.10f,0.13f);
//     R_filledEllipse(0.f,-0.05f,0.72f,0.80f,80);

//     R_col(0.18f,0.17f,0.20f);
//     glLineWidth(1.f);
//     for(int i=0;i<8;i++){
//         float ry=0.34f-i*0.10f;
//         float rw=0.56f-fabsf(ry)*0.20f;
//         R_outlineEllipse(0.f,ry,rw,0.055f,36);
//     }
// }

// /* ── Lung shape — Catmull-Rom spline (Ch.4) ─────────────────── */
// std::vector<float> LungSimulation::buildLungFan(
//     float cx,float cy,float sc,int side,float breath)
// {
//     float m=(float)side; /* +1=right, -1=left — mirror transform Ch.3 */
...
//         { 0.00f, 0.24f},{ 0.03f, 0.30f}, /* close */
//     };
//     /* Left lung: deeper cardiac notch (anatomically accurate) */
//     if(side==-1){ kp[12]={-0.14f,-0.06f}; kp[13]={-0.16f,0.08f}; kp[14]={-0.10f,0.18f}; }

//     /* Get Catmull-Rom points in local space */
//     auto base = G_catmullRomFan(kp, 0.16f*m, -0.04f, 14);

//     /* Apply 2D transform: scale + mirror + translate (Ch.3) */
...
//             float spr=0.020f+seed*0.040f;
//             float dk=0.030f+sev*0.065f;
...
//     glLineWidth(2.6f);
//     R_draw(GL_LINE_LOOP,outline);
//     glLineWidth(1.f);

//     /* 8. Bronchial tree — line-strip rendering */
//     R_col(vc);
//     glLineWidth(3.5f);
//     float pb[]={cx+0.03f*m*sc,cy+0.23f*sc, cx+0.07f*m*sc,cy+0.11f*sc};
//     R_lineStrip(pb,2);
//     glLineWidth(2.8f);
//     float dk2[]={cx+0.07f*m*sc,cy+0.11f*sc,
//                  cx+0.14f*m*sc,cy-0.06f*sc,
//                  cx+0.12f*m*sc,cy-0.26f*sc,
//                  cx+0.10f*m*sc,cy-0.39f*sc};
//     R_lineStrip(dk2,4);
//     glLineWidth(2.2f);
//     float ub[]={cx+0.07f*m*sc,cy+0.11f*sc,
//                 cx+0.16f*m*sc,cy+0.23f*sc,
//                 cx+0.20f*m*sc,cy+0.35f*sc,
//                 cx+0.18f*m*sc,cy+0.43f*sc};
//     R_lineStrip(ub,4);
//     glLineWidth(2.0f);
//     float mb[]={cx+0.14f*m*sc,cy-0.06f*sc,
//                 cx+0.26f*m*sc,cy-0.02f*sc,
//                 cx+0.34f*m*sc,cy-0.11f*sc};
//     R_lineStrip(mb,3);
//     glLineWidth(1.4f);
//     float sb1[]={cx+0.16f*m*sc,cy+0.23f*sc,cx+0.28f*m*sc,cy+0.21f*sc,cx+0.36f*m*sc,cy+0.10f*sc};
//     R_lineStrip(sb1,3);
//     float sb2[]={cx+0.20f*m*sc,cy+0.35f*sc,cx+0.29f*m*sc,cy+0.37f*sc};
//     R_lineStrip(sb2,2);
//     float sb3[]={cx+0.26f*m*sc,cy-0.02f*sc,cx+0.33f*m*sc,cy+0.07f*sc};
//     R_lineStrip(sb3,2);
//     float sb4[]={cx+0.12f*m*sc,cy-0.26f*sc,cx+0.22f*m*sc,cy-0.31f*sc};
//     R_lineStrip(sb4,2);
//     float sb5[]={cx+0.10f*m*sc,cy-0.39f*sc,cx+0.19f*m*sc,cy-0.41f*sc};
//     R_lineStrip(sb5,2);
//     float sb6[]={cx+0.10f*m*sc,cy-0.39f*sc,cx+0.12f*m*sc,cy-0.45f*sc};
//     R_lineStrip(sb6,2);
//     glLineWidth(1.f);
// }

// /* ── Trachea + main bronchi — Bezier arcs (Ch.4) ────────────── */
// void LungSimulation::drawTrachea(float cx,float cy,float sc){
//     Col3 vc=bronchi();
//     float tw=0.038f*sc;
//     float ttop=cy+0.80f*sc, tbot=cy+0.30f*sc; /* carina */
...
//     for(int i=0;i<10;i++){
//         float ry=ttop-(ttop-tbot)*(float)i/9.f;
//         float r[]={cx-tw,ry, cx+tw,ry};
//         R_draw(GL_LINES,r,2);
//     }
...
//     auto lbez=G_bezier(cx,tbot, cx-0.05f*sc,tbot-0.09f*sc,
//                        lhx+0.08f*sc,hilY+0.07f*sc, lhx,hilY);
//     auto rbez=G_bezier(cx,tbot, cx+0.05f*sc,tbot-0.09f*sc,
//                        rhx-0.08f*sc,hilY+0.07f*sc, rhx,hilY);

//     glLineWidth(9.f);
//     R_col(vc.r*0.38f,vc.g*0.38f,vc.b*0.38f);
//     R_lineStrip(lbez); R_lineStrip(rbez);
//     glLineWidth(6.f);
//     R_col(vc.r*0.62f,vc.g*0.62f,vc.b*0.62f);
//     R_lineStrip(lbez); R_lineStrip(rbez);
//     glLineWidth(2.5f);
//     R_col(vc);
//     R_lineStrip(lbez); R_lineStrip(rbez);
...
//     R_col(0.97f,0.96f,0.91f);
//     R_filledRect(x,y-h,x+len,y);
//     R_col(0.90f,0.50f,0.12f);
//     R_filledRect(x-flt,y-h,x,y);
...
//     R_outlineRect(x-flt,y-h,x+len,y);
//     glLineWidth(1.f);
// }

// /* ── Stage panel (right side) ───────────────────────────────── */
// void LungSimulation::drawStagePanel(){
//     float panelW=0.52f, panelX=1.52f-panelW;
//     float panelTop=0.84f, panelBot=-0.82f;
//     float itemH=(panelTop-panelBot)/(float)LUNG_STAGE_COUNT;

//     R_col(0.08f,0.07f,0.10f,0.93f);
//     R_filledRect(panelX,panelBot,panelX+panelW,panelTop);
//     R_col(0.22f,0.20f,0.28f); glLineWidth(1.5f);
//     R_outlineRect(panelX,panelBot,panelX+panelW,panelTop);
//     glLineWidth(1.f);

//     Col3 sc=stageCol();
//     R_col(sc.r*0.40f,sc.g*0.40f,sc.b*0.40f);
//     R_filledRect(panelX,panelTop-0.060f,panelX+panelW,panelTop);

//     for(int i=0;i<LUNG_STAGE_COUNT;i++){
//         const StageInfo& si=LUNG_STAGES[i];
//         (void)si;
//         float rowTop=panelTop-0.060f-i*itemH;
//         float rowBot=rowTop-itemH+0.005f;
//         bool isActive=(i==anim.stage), isDone=(i<anim.stage);

//         if(isActive)      R_col(sc.r*0.28f,sc.g*0.28f,sc.b*0.28f,0.95f);
//         else if(isDone)   R_col(0.13f,0.12f,0.16f,0.80f);
//         else              R_col(0.10f,0.09f,0.12f,0.60f);
//         R_filledRect(panelX+0.008f,rowBot,panelX+panelW-0.008f,rowTop);

//         if(isActive){
//             R_col(sc); glLineWidth(2.f);
//             R_outlineRect(panelX+0.008f,rowBot,panelX+panelW-0.008f,rowTop);
//             float arrowX=panelX+panelW-0.032f, amid=(rowTop+rowBot)*0.5f;
//             float arr[]={arrowX,amid+0.016f, arrowX+0.026f,amid, arrowX,amid-0.016f};
//             R_draw(GL_TRIANGLE_FAN,arr,3);
//             glLineWidth(1.f);
//         }

//         Col3 rc=SC[i];
//         float inten=isActive?1.f:(isDone?0.55f:0.25f);
//         R_col(rc.r*inten,rc.g*inten,rc.b*inten);
//         R_filledRect(panelX+0.008f,rowBot,panelX+0.026f,rowTop);

//         /* Stage label bar */
//         float lbr=isActive?1.f:(isDone?0.75f:0.42f);
//         R_col(lbr,lbr*0.95f,lbr*0.90f);
//         R_filledRect(panelX+0.032f,rowTop-0.026f,
//                      panelX+0.032f+0.18f*(isActive?1.f:0.75f),rowTop-0.040f);

//         /* Time label bar */
//         R_col(rc.r*inten*0.9f,rc.g*inten*0.9f,rc.b*inten*0.9f);
//         R_filledRect(panelX+0.032f,rowTop-0.044f,panelX+0.032f+0.12f,rowTop-0.056f);

//         /* Condition bar */
//         float ca=isActive?0.70f:0.35f;
//         R_col(0.65f*ca,0.62f*ca,0.70f*ca);
//         R_filledRect(panelX+0.032f,rowBot+0.010f,
//                      panelX+0.032f+0.14f*(isActive?1.f:0.65f),rowBot+0.022f);

//         /* Progress fill inside active row */
//         if(isActive){
//             float prog=anim.stageFrac();
//             R_col(0.15f,0.14f,0.18f);
//             R_filledRect(panelX+0.032f,rowBot+0.004f,panelX+panelW-0.036f,rowBot+0.014f);
//             R_col(sc,0.80f);
//             float fillW=(panelW-0.068f)*prog;
//             R_filledRect(panelX+0.032f,rowBot+0.004f,panelX+0.032f+fillW,rowBot+0.014f);
//         }

//         R_col(0.20f,0.18f,0.24f); glLineWidth(1.f);
//         float sep[]={panelX+0.008f,rowBot, panelX+panelW-0.008f,rowBot};
//         R_draw(GL_LINES,sep,2);
//     }
// }

// /* ── Bottom bar — damage spectrum + description ─────────────── */
// void LungSimulation::drawBottomBar(){
//     Col3 sc=stageCol();

//     R_col(0.08f,0.07f,0.11f,0.95f);
//     R_filledRect(-1.58f,-0.98f,1.28f,-0.70f);
//     R_col(sc.r*0.30f,sc.g*0.30f,sc.b*0.30f);
//     R_outlineRect(-1.58f,-0.98f,1.28f,-0.70f);

//     /* Stage/time label bar */
//     R_col(sc.r*0.55f,sc.g*0.55f,sc.b*0.55f);
//     R_filledRect(-1.55f,-0.74f,-0.55f,-0.726f);

//     /* Damage spectrum bar */
//     float bx1=-1.55f,bx2=1.24f,by=-0.82f,bh=0.025f;
//     R_col(0.12f,0.11f,0.15f);
//     R_filledRect(bx1,by-bh,bx2,by);
...
//         R_col(t0,1.f-t0,0.1f,0.85f);
//         R_filledRect(bx1+bw*t0,by-bh, bx1+bw*tU,by);
//     }
//     /* Pip */
//     R_col(1,1,1);
//     float px=bx1+bw*total;
//     R_filledRect(px-0.007f,by-bh-0.007f,px+0.007f,by+0.007f);

//     /* "Healthy" / "Damaged" label bars */
//     R_col(0.25f,0.72f,0.28f,0.80f);
//     R_filledRect(bx1,by-bh-0.023f,bx1+0.08f,by-bh-0.013f);
//     R_col(0.72f,0.10f,0.10f,0.80f);
//     R_filledRect(bx2-0.08f,by-bh-0.023f,bx2,by-bh-0.013f);

//     /* Description bar */
//     R_col(sc.r*0.60f,sc.g*0.60f,sc.b*0.60f);
//     R_filledRect(-1.55f,-0.96f,-0.85f,-0.946f);

//     glLineWidth(1.2f);
//     R_col(0.35f,0.32f,0.42f);
//     R_outlineRect(bx1,by-bh,bx2,by);
//     glLineWidth(1.f);
// }

// /* ── Algorithm citation bar ─────────────────────────────────── */
// void LungSimulation::drawAlgorithmBar(){
//     R_col(0.10f,0.10f,0.14f,0.90f);
//     R_filledRect(-2.f,-1.f,2.f,-0.985f);
//     Col3 sc=stageCol();
//     float lx[]={0.f,0.26f,0.52f,0.78f,1.04f,1.30f};
//     for(int i=0;i<6;i++){
//         R_col(sc.r*0.50f,sc.g*0.50f,sc.b*0.50f);
//         R_filledRect(-1.58f+lx[i],-0.999f,-1.58f+lx[i]+0.22f,-0.988f);
//     }
// }
...
void LungSimulation::drawStagePanel(){
    ...
    for(int i=0;i<LUNG_STAGE_COUNT;i++){
        ...
        /* Stage label: "Stage 1" */
        float labelSz=0.028f;
        R_col(1.f*alpha,0.96f*alpha,0.88f*alpha);
        T_drawText(tx, rowTop-labelSz-0.004f, labelSz, si.label);

        /* Time label e.g. "| 1 Month" */
        R_col(rc.r*inten,rc.g*inten,rc.b*inten,alpha);
        char timeBuf[32]; snprintf(timeBuf,sizeof(timeBuf),"| %s",si.timeLabel);
        float lblW=T_width(si.label,labelSz)+0.010f;
        T_drawText(tx+lblW, rowTop-labelSz-0.004f, labelSz, timeBuf);

        /* Condition text below */
        float condSz=0.022f;
        R_col(0.72f*alpha,0.70f*alpha,0.78f*alpha);
        T_drawText(tx, rowTop-labelSz-0.008f-condSz, condSz, si.condition);
...
void LungSimulation::drawBottomBar(){
    ...
    /* "Healthy" / "Damaged" labels */
    R_col(0.28f,0.80f,0.32f); T_drawText(barX1,barY-0.028f,0.022f,"Healthy");
    R_col(0.80f,0.18f,0.14f); T_drawRight(barX2,barY-0.028f,0.022f,"Damaged");

    /* Description */
    R_col(sc.r*0.90f,sc.g*0.90f,sc.b*0.90f);
    T_drawText(bx1+0.04f,by1+0.016f,0.024f,si.description);
...
void LungSimulation::drawAlgorithmBar(){
    ...
    T_drawText(-1.55f,y+0.005f,0.020f,"Algorithm:");

    float startX=-1.55f+T_width("Algorithm:",0.020f)+0.015f;
    const char* algos[]={"Bezier(Ch.4)","Scanline(Ch.2)","Transforms(Ch.3)",
                         "Gouraud(Ch.6)","Keyframe(Ch.7)","DirectMotion(Ch.7.4)"};
    R_col(0.62f,0.60f,0.72f);
    float cx=startX;
    for(int i=0;i<6;i++){
        T_drawText(cx,y+0.005f,0.018f,algos[i]);
        cx+=T_width(algos[i],0.018f)+0.020f;
        if(i<5){ /* separator dot */
            R_col(sc.r*0.6f,sc.g*0.6f,sc.b*0.6f);
            R_filledEllipse(cx-0.010f,y+0.010f,0.004f,0.004f,8);
            R_col(0.62f,0.60f,0.72f);
        }
    }
}